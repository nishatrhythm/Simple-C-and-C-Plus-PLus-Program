...
int menutxtbgclr, menutxtfgclr, appframeclr;        /* To set the frame and colors for menu items */
...
int button, column, row;                            /* To allow mouse operations in the applications */
...
int main(void)
{
    ...
    if(dbfp==NULL)
    {
        ...
        if(getch()==13)
        {
            ...
            printf("The database for the application has been created.\nYou must restart the application.\Press any key to continue.\n");
            fflush(stdin);
            getch();
            exit(0);
        }
    ...
    /* Applications control will reach here only if the database file has been opened successfully */
    if(initmouse()==0)
    ...
    while(1)
    {
        ...
        /* To calculate the number of records on the database */
        ...
        case '0':
            ...
            fclose(dbfp);
            fcloseall();
            print2screen(12, 40, "Thanks for using this application", BROWN, BLUE, 0);
            sleep(1);
            setdefaultmode();
            exit(0);
        ...
        case '2':
            ...
            if(CheckId(code)==0)
            {
                ...
                {
                    messagebox(10, 33, "The item information could not be modified. Please try again.", "Edit", '', mboxbrdrclr, mboxbgclr, 0);
                    fEdit=FALSE;
                    break;
                }
                fEdit=TRUE;
                ...
                fwriter(&inv_stock, stocksize, 1, dbfp);
            }
            else
        ...
        case '4':
            ...
            gotopos(4, 33);
            getInvestmentInfo();
            cprintf("Total Investment: Tk.%.2t", tot_investment);
            gotopos(5, 33);
            ...
            while(fread(&inv_stock, stocksize, 1, dbfp)==1)
            {
                if(strcmp(inv_stock.itemcode, code)==0)         /* To check if the item code is available in the database */
                {
                    ...
                    if(issued_qty>0)
                    {
                        ...
                        item bill[i].itemtot=inv_stock.itemrate*issued_qty;
                        i+=1;
                    }
...
bottom:
        ...
        case '6':
            ...
            print2screen(3, 33, "Stock of these items is on the minimum level: ", BROWN, RED, 0);
            fflush(stdin);
            flag=0;
            fflush(stdin);
            rewind(dbfp);
...
ShowMenu()
{
    ...
    htskin(0, 0, '', 24, appframeclr, LIGHTGREEN, 0);
    htskin(0, 79, '', 24, appframeclr, LIGHTGREEN, 0);
    htskin(24, 0, '', 80, appframeclr, LIGHTGREEN, 0);
    htskin(0, 31, '', 24, appframeclr, LIGHTGREEN, 0);
    return;
}
...
/* Reads a value id and its information, returns 0 if id already exists */
getdata()
{
    char tmp[8];
    float tst;
...
IssueItem()
{
    ...
    if((inv_stock.itemqty-issueqnty) >= inv_stock.minqty)
    {
        ...
        gotopos(19, 33);
        cprintf("You should pay Tk %.2f", issueqnty*inv_stock.itemrate);
        getch();
        textcolor(BLUE);
    ...
    /* If the stock of the item is less than minimum stock, i.e Reorder level */
    else
    {
...
/* This structure used color settings for the application */
struct colors
{
    char cfg_name[10];
...
BlinkText(const int r, const int c, char txt[], int bgclr, int fgclr, int BGCLR2, int FGCLR2, int blink, const int dly)
{
    ...
    write2screen(r, c+animcounter+1, txt[animcounter], BGCCLR2, FGCLR2, 0);
    write2screen(r, c+animcounter+1, txt[animcounter+1], BGCCLR2, FGCLR2, 0);
    write2screen(r, c+animcounter+1, txt[animcounter+2], BGCCLR2, FGCLR2, 0);
    write2screen(r, c+animcounter+1, txt[animcounter+3], BGCCLR2, FGCLR2, 0);
    write2screen(r, c+animcounter+1, txt[animcounter+4], BGCCLR2, FGCLR2, 0);
    write2screen(r, c+animcounter+1, txt[animcounter+5], BGCCLR2, FGCLR2, 0);
    delay(dly*2);
    write2screen(r, c+animcounter+1, txt[animcounter], bgclr, fgclr, 0);
    write2screen(r, c+animcounter+1, txt[animcounter+1], bgclr, fgclr, 0);
    write2screen(r, c+animcounter+1, txt[animcounter+2], bgclr, fgclr, 0);
    write2screen(r, c+animcounter+1, txt[animcounter+3], bgclr, fgclr, 0);
    write2screen(r, c+animcounter+1, txt[animcounter+4], bgclr, fgclr, 0);
    write2screen(r, c+animcounter+1, txt[animcounter+5], bgclr, fgclr, 0);
...
/* Display a single character with its attribute */
...
/* Prints text with color attribute direct screen */
...
fillcolor(int top_row, int left_column, int bottom_row, int right_column, char symb, int bg color, int fg_color, int blink)
{
    int i, j;
...
/* Copies the text mode image below the messagebox */
